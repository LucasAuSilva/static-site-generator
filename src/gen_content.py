...
def generate_page(base_path, from_path, template_path, dest_path):
    ...
    new_page_contents = new_page_contents.replace("href=\"", f"href=\"{base_path}")
    new_page_contents = new_page_contents.replace("src=\"", f"src=\"{base_path}")
    md_file.close()
...
def generate_page_recursive(base_path, path_content, template_path, dest_path):
    ...
    for filename in os.listdir(path_content):
        ...
        if os.path.isfile(from_path):
            generate_page(base_path, from_path, template_path, new_path.replace(".md", ".html"))
        else:
            generate_page_recursive(base_path, from_path, template_path, new_path)
